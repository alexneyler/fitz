...
2. If your change affects commands, CLI behavior, or docs, run the `.agents/skills/update-docs` skill before committing. Only update docs for functional changes (new/removed/renamed commands, changed arguments, altered behavior). Documentation should stay succinct and user-facing; avoid implementation details unless required for command usage. Cosmetic or internal-only changes (animations, refactors, style tweaks) do not require doc updates.
