...
- `fitz config [--global] <command>` — get and set configuration values. Config is stored at `~/.fitz/<owner>/<repo>/config.json` (repo-level) or `~/.fitz/config.json` (global). Defaults: `model=gpt-5.3-codex`, `agent=copilot-cli`, `branch-open-mode=zellij`. Repo config overrides global, which overrides built-in defaults.
  ...
  - Valid keys: `model` (passed as `--model` to Copilot CLI on every invocation), `agent` (agent framework; only `copilot-cli` supported today), `branch-open-mode` (`zellij` or `legacy`).
